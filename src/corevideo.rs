...
pub type CVDisplayLinkOutputCallback = Option<extern "system" fn(displayLink: CVDisplayLinkRef, inNow: *const CVTimeStamp,
    inOutputTime: *const CVTimeStamp, flagsIn: CVOptionFlags, flagsOut: *mut CVOptionFlags, displayLinkContext: *mut c_void)
    -> CVReturn>;
...
impl CVDisplayLink {
    ...
    pub fn new_for_active_displays() -> Result<ExternalRc<Self>, CVReturn> {
        ...
        if r != 0 { Ok(unsafe { ExternalRc::with_fn(h, CVDisplayLinkRetain, CVDisplayLinkRelease) }) } else { Err(r) }
    }
    ...
    pub fn set_output_callback(&mut self, callback: CVDisplayLinkOutputCallback, user: *mut c_void)
        -> Result<(), CVReturn> {
    ...
    pub fn is_running(&self) -> bool { unsafe { CVDisplayLinkIsRunning(self as *const _ as _) } }
}

#[link(name = "QuartzCore", kind = "framework")]
extern "system" {
    fn CVDisplayLinkCreateWithCGDisplay(displayID: ::CGDirectDisplayID, displayLinkOut: *mut CVDisplayLinkRef) -> CVReturn;
    fn CVDisplayLinkCreateWithActiveCGDisplays(displayLinkOut: *mut CVDisplayLinkRef) -> CVReturn;
    fn CVDisplayLinkSetOutputCallback(displayLink: CVDisplayLinkRef, callback: CVDisplayLinkOutputCallback,
        userInfo: *mut c_void) -> CVReturn;
