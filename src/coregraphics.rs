...
impl CGPath {
    ...
    pub fn new_mutable() -> Result<ExternalRc<Self>, ()> { unsafe { Self::own_from(CGPathCreateMutable()).ok_or(()) } }
    ...
    pub fn apply<F>(&self, mut callback: F) where F: FnMut(&CGPathElement) {
        extern "C" fn cb_wrap<F>(ctx: *mut c_void, element: *const CGPathElement) where F: FnMut(&CGPathElement) {
            let callback = unsafe { (ctx as *mut F).as_mut().unwrap() };
            callback(unsafe { element.as_ref().unwrap() });
        }
        unsafe { self.apply_unchecked(&mut callback as *mut F as _, cb_wrap::<F>); }
    }
}
#[repr(C)] #[derive(Clone, Debug, PartialEq, Eq)]
pub struct CGPathElement { pub type_: CGPathElementType, pub points: *mut CGPoint }
#[repr(u32)] #[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum CGPathElementType {
    MoveToPoint, AddLineToPoint, AddQuadCurveToPoint, AddCurveToPoint, CloseSubpath
}
